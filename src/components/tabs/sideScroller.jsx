...
const SideScroller = ({ children }) => {
    ...
    return (
        <div className="row">
            <div onClick={() => handleScroll("left")} className={`scroll-left${displayButton("left") ? "" : " invisible"}`}>
                <Media.IndentedArrowSVG />
            </div>
            {children && cloneElement(children, { ref, className: `${children.props.className ?? ""} side-scroller` })}
            <div onClick={() => handleScroll("right")} className={`scroll-right${displayButton("right") ? "" : " invisible"}`}>
                <Media.IndentedArrowSVG />
            </div>
        </div>
    );
